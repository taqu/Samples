...
namespace render
{
    ...
    void ShaderManager::initializeImpl()
    {
        ...
        //
        compiler.compileShader(vs, ps, Shader_Ocean);
        LASSERT(vs.valid() && ps.valid());
        entries_[Shader_Ocean].vs_ = LIME_NEW OceanVS(vs);
        entries_[Shader_Ocean].ps_ = LIME_NEW OceanPS(ps);

        //
        compiler.compileShaderPS(ps, Shader_OceanWire);
        LASSERT(ps.valid());
        entries_[Shader_OceanWire].ps_ = LIME_NEW OceanWirePS(ps);
