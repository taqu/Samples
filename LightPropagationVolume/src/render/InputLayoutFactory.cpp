...
#include "shader/Layout_PNUBone.byte"
        ...
        static const u8 LayoutData_Ocean[] =
        {
#include "shader/Layout_Ocean.byte"
        };
    }
    ...
    bool InputLayoutFactory::initialize()
    {
        ...
        {//Layout_Ocean
            ...
            compiler.resize(sizeof(LayoutData_Ocean));
            u32 size = compiler.decompress(sizeof(LayoutData_Ocean), LayoutData_Ocean);
            if(size>0){
                layouts_[Layout_Ocean] = lgraphics::InputLayout::create(layout, 1, compiler.getData(), size);
            }
        }
        return true;
    }
